...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Full‐parameter finetune on normal reasoning dataset with bf16 and multi‐GPU."
    )
    parser.add_argument(
        "--dataset_dir",
        type=str,
...
def main():
    ...
    #    - Pads all “input_ids” & “attention_mask” to longest in the batch.
    #    - Fills any padded‐label positions with -100.
    # ────────────────────────────────────────────────────────────────────────────
    def my_collator(features):
        return collate_batch_for_causal_lm(features, tokenizer.pad_token_id)

    model_tag = args.base_model
    output_dir = f"Plain-{model_tag}-sft-only"
